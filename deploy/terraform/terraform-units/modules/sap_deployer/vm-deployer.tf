...
resource "azurerm_network_interface" "deployer" {
  ...
  ip_configuration                       {
                                          ...
                                          private_ip_address            = var.deployer.use_DHCP ? cidrhost(local.management_subnet_deployed_prefixes[0], tonumber(count.index) + 4) : var.deployer.private_ip_address[count.index]
